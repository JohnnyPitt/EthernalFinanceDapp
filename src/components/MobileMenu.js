...
const MobileMenu=(props)=>{
  ...
  return (
    <div className="mobile-menu  d-flex justify-content-around">
    	<div className={props.view === 1 ? 'selected-tab' : ''} onClick={()=>{props.changeView(1)}}>
    		<div>{props.view === 1 ? <Rewards2/> : <Rewards1/>}</div>
    		<div className='text-gray'>Rewards</div>
    	</div>
    	<div className={props.view === 2 ? 'selected-tab' : ''} onClick={()=>{props.changeView(2)}}>
    		<div>{props.view === 2 ? <Exchange2/> : <Exchange1/>}</div>
    		<div className='text-gray'>Exchange</div>
    	</div>
		<div className={props.view === 3 ? 'selected-tab' : ''} onClick={()=>{props.changeView(3)}}>
    		<div>{props.view === 3 ? <Rewards2/> : <Rewards1/>}</div>
    		<div className='text-gray'>Governance</div>
    	</div>
		<div className={props.view === 4 ? 'selected-tab' : ''} onClick={()=>{props.changeView(4)}}>
    		<div>{props.view === 4 ? <Rewards2/> : <Rewards1/>}</div>
